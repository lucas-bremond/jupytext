...
async def test_load_save_rename(nb_file, cm, tmpdir):
    tmp_ipynb = "notebook.ipynb"
...
async def test_save_load_paired_md_notebook(nb_file, cm, tmpdir):
    tmp_ipynb = "notebook.ipynb"
    tmp_md = "notebook.md"
...
async def test_save_load_paired_md_pandoc_notebook(nb_file, cm, tmpdir):
    tmp_ipynb = "notebook.ipynb"
    tmp_md = "notebook.md"
...
async def test_save_load_paired_qmd_notebook(nb_file, cm, tmpdir):
    tmp_ipynb = "notebook.ipynb"
...
async def test_pair_plain_script(py_file, cm, tmpdir, caplog):
    tmp_py = "notebook.py"
    tmp_ipynb = "notebook.ipynb"
...
async def test_load_save_rename_nbpy(nb_file, cm, tmpdir):
    tmp_ipynb = "notebook.ipynb"
...
async def test_load_save_py_freeze_metadata(script, cm, tmpdir):
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py"))
async def test_load_save_rename_notebook_with_dot(nb_file, cm, tmpdir):
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py"))
async def test_load_save_rename_nbpy_default_config(nb_file, cm, tmpdir):
    tmp_ipynb = "notebook.ipynb"
    tmp_nbpy = "notebook.nb.py"
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py"))
async def test_load_save_rename_non_ascii_path(nb_file, cm, tmpdir):
...
async def test_outdated_text_notebook(nb_file, cm, tmpdir):
    # 1. write py ipynb
    cm.formats = "py,ipynb"
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py")[:1])
async def test_reload_notebook_after_jupytext_cli(nb_file, cm, tmpdir):
...
async def test_load_save_percent_format(nb_file, cm, tmpdir):
    tmp_py = "notebook.py"
...
async def test_save_to_percent_format(nb_file, cm, tmpdir):
    tmp_ipynb = "notebook.ipynb"
    tmp_jl = "notebook.jl"
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py"))
async def test_save_using_preferred_and_default_format_170(nb_file, cm, tmpdir):
    nb = read(nb_file)
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py"))
async def test_open_using_preferred_and_default_format_174(nb_file, cm, tmpdir):
    tmp_ipynb = str(tmpdir.join("notebook.ipynb"))
    tmp_py = str(tmpdir.join("python/notebook.py"))
...
async def test_kernelspec_are_preserved(nb_file, cm, tmpdir):
    tmp_ipynb = str(tmpdir.join("notebook.ipynb"))
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py"))
async def test_save_to_light_percent_sphinx_format(nb_file, cm, tmpdir):
    tmp_ipynb = "notebook.ipynb"
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py"))
async def test_pair_notebook_with_dot(nb_file, cm, tmpdir):
    # Reproduce issue #138
    tmp_py = "file.5.1.py"
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py")[:1])
async def test_preferred_format_allows_to_read_others_format(nb_file, cm, tmpdir):
    # 1. write py ipynb
    tmp_ipynb = "notebook.ipynb"
    tmp_nbpy = "notebook.py"
...
async def test_save_in_auto_extension_global(nb_file, cm, tmpdir):
    # load notebook
    nb = jupytext.read(nb_file)
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb"))
async def test_save_in_auto_extension_global_with_format(nb_file, cm, tmpdir):
    # load notebook
    nb = jupytext.read(nb_file)
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb"))
async def test_save_in_auto_extension_local(nb_file, cm, tmpdir):
    # load notebook
    nb = jupytext.read(nb_file)
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb"))
async def test_save_in_pct_and_lgt_auto_extensions(nb_file, cm, tmpdir):
    # load notebook
    nb = jupytext.read(nb_file)
...
async def test_metadata_filter_is_effective(nb_file, cm, tmpdir):
    nb = jupytext.read(nb_file)
    tmp_ipynb = "notebook.ipynb"
...
async def test_local_format_can_deactivate_pairing(nb_file, ext, cm, tmpdir):
...
async def test_global_pairing_allows_to_save_other_file_types(nb_file, cm, tmpdir):
...
async def test_python_kernel_preserves_R_files(nb_file, cm, tmpdir):
...
@requires_sphinx_gallery
async def test_rst2md_option(cm, tmpdir):
    tmp_py = str(tmpdir.join("notebook.py"))
    ...
    nb2 = (await cm.get("notebook.py"))["content"]
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py")[:1])
async def test_set_then_change_auto_formats(cm, tmpdir, nb_file):
    tmp_ipynb = str(tmpdir.join("nb.ipynb"))
    tmp_py = str(tmpdir.join("nb.py"))
...
@pytest.mark.parametrize("nb_file", list_notebooks("ipynb_py"))
async def test_share_py_recreate_ipynb(cm, tmpdir, nb_file):
    tmp_ipynb = str(tmpdir.join("nb.ipynb"))
    tmp_py = str(tmpdir.join("nb.py"))
...
async def test_new_untitled(cm, tmpdir):
    ...
    assert (await cm.new_untitled(type="notebook"))["path"] == untitled + "3.ipynb"
...
async def test_jupytext_jupyter_fs_metamanager(tmpdir):
    ...
    await cm.save(notebook_model(nb), osfs + ":notebook.ipynb")
    await cm.save(notebook_model(nb), osfs + ":text_notebook.md")

    # list the directory
    directory = await cm.get(osfs + ":/")
    ...
    # get the files
    model = await cm.get(osfs + ":/text.md", type="file")
    assert model["type"] == "file"
    assert model["content"] == text

    model = await cm.get(osfs + ":text.md", type="notebook")
    assert model["type"] == "notebook"
    ...
    for nb_file in ["notebook.ipynb", "text_notebook.md"]:
        model = await cm.get(osfs + ":" + nb_file)
        assert model["type"] == "notebook"
...
async def test_config_jupytext_jupyter_fs_meta_manager(tmpdir):
    ...
    await cm.save(notebook_model(nb), osfs + ":script.py")
    await cm.save(notebook_model(nb), osfs + ":text_notebook.md")
    await cm.save(notebook_model(nb), osfs + ":notebook.ipynb")
...
async def test_timestamp_is_correct_after_reload_978(cm, tmp_path, python_notebook):
